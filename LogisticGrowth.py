...
def calculate_logistic_function_params_constrained(values, x0):
    days = np.array(np.arange(len(values)), dtype=float)
    fit_params, cov = curve_fit(logistic_funct_constrained, days, values)
    return fit_params
...
def logistic_funct_constrained(x, L, k):
    return L / (1 + np.exp(-k * (x - 39)))
    # def constrained_logistic_func(x, L, k):
    #     return L / (1 + np.exp(-k * (x - x0)))
    # return constrained_logistic_func
